...
def docx_table_to_html_md_tabel(src_path: str = "example_input/messplan.docx",
                                dst_path: Optional[str] = "example_input/gottesdienst.html") -> Optional[str]:
    """
    Converts DOCX with a table to an HTML with table. So we can read it afterwards.
    :param src_path:
    :param dst_path:
    :return md_text if dst_path == None
    """
    try:
        ...
        print('Unexpected error while converting docx table to html-md table:', e)
...
def get_gottesdienstplan_from_html(path: str = "example_input/gottesdienst.html") -> pd.DataFrame:
    ...
    # rename columns
    gottesdienst_table.rename(columns={0: DATUM, 1: ZEIT, 3: GOTTESDIENST}, inplace=True)
    ...
    gottesdienst_table = gottesdienst_table.reindex(columns=[TAG, DATUM, ZEIT, GOTTESDIENST])
...
def get_messdiener_from_csv(path: str = "example_input/messdiener.csv") -> pd.DataFrame:
    ...
    for black_list in [BLACK_LIST_TAGE, BLACK_LIST_PERSONEN]:
        messdiener_table[black_list] = [
            *map(lambda day_str: '' if day_str is None or str(day_str) == 'nan' else day_str,
                 messdiener_table[black_list])]
...
def get_gottesdienst_arten_from_json(path: str = "example_input/gottesdienst-arten.json") -> dict:
    ...
    """
    with open(path) as gottesdienst_arten_file:
...
def export_table_to_excel(export_table: pd.DataFrame, name: str = "table") -> None:
    ...
    :param name: path to the new document
    """
    sheet_name = f'Messdienerplan Stand {datetime.datetime.now().strftime("%d.%m.%Y")}'
    writer = pd.ExcelWriter(name, engine='xlsxwriter')
    export_table.to_excel(excel_writer=writer, sheet_name=sheet_name, index=False)
    ...
    for i, day, date in zip(range(len(export_table[TAG])), export_table[TAG], export_table[DATUM]):
        if last_date == date:
            ...
            if length > 1:
                worksheet.merge_range(f'A{i + 2 - length}:A{i + 1}', export_table[TAG][export_table_index[i - 1]])
                worksheet.merge_range(f'B{i + 2 - length}:B{i + 1}', export_table[DATUM][export_table_index[i - 1]])
            last_date = date
            length = 1
        i += 1
    if length > 1:
